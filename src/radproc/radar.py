...
def get_gate_altitude(radar: Radar, sweep: int, x: float, y: float,
                      asl=False) -> float:
    """
    Get the altitude of the closest gate to the specified (x,y) coordinates.
    ...
    asl: bool
        Set to True to measure altitude from the sea level. Otherwise it is
        measured from the radar antenna level.
    ...
    if asl:
        _, _, gate_z = radar.get_gate_lat_lon_alt(sweep)
